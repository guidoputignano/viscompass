...
export default async function PrivatePillarA(){
 ...
 // The private workbook is not activated: the fact table has not been created
 // and activation has not been approved. Say so instead of querying a relation
 // that does not exist and surfacing an error boundary.
 if(!PRIVATE_PILLAR_A_ACTIVE)return <NotActivated/>;
 ...
 if(error?.code===UNDEFINED_TABLE)return <NotActivated/>;
...
// Truthful empty state. No demonstration data is substituted: AGENTS.md requires
// synthetic values to be impossible to mistake for real ones, and the honest
// answer here is that the workbook is not yet available to this organization.
function NotActivated(){
 return <div className="space-y-4">
  <PageHeader eyebrow="Pillar A · area riservata" title="Il workbook della tua organizzazione" description="Antibiotici J01 · fonti 2023–2025 · spesa CF, costo CMR e DDD mantenuti distinti." period="2023–2025" scope="Non ancora attivo"/>
  <section className="rounded-xl border bg-card p-6">
   <h2 className="text-lg font-semibold">Workbook riservato non ancora attivo</h2>
   <p className="mt-3 text-sm leading-relaxed text-muted-foreground">Le analisi riservate sono pronte ma non sono state attivate per questa organizzazione: la tabella dei dati non è ancora stata creata e l’attivazione richiede l’approvazione del titolare. Nessun dato dimostrativo viene mostrato al suo posto.</p>
   <p className="mt-3 text-sm leading-relaxed text-muted-foreground">Le evidenze pubbliche restano disponibili nella sezione <span className="font-medium">Pillar A · fonti pubbliche</span>.</p>
  </section>
 </div>;
}
