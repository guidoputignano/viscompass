...
import { getBiosimilarComparison, resolveScope } from "@/lib/dashboard-review/queries";
...
export default async function BiosimilarToEurosPage({
  ...
  const rows = await getBiosimilarComparison(scope);
      ...
      <Card>
        <CardContent className="p-6">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            Opportunità stimata nell&apos;ambito selezionato
          </p>
        ...
        <CardContent className="overflow-x-auto p-0">
          {rows.length === 0 ? (
            <p className="p-8 text-sm text-muted-foreground">
              Nessun dato disponibile nell&apos;ambito selezionato.
            </p>
          ) : (
