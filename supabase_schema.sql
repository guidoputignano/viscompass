...
create unique index if not exists organization_invitations_one_pending_per_email_org
  on organization_invitations (lower(email), org_code)
...
drop policy if exists "read invitations addressed to own email" on organization_invitations;
create policy "read invitations addressed to own email"
  on organization_invitations for select to authenticated
  using (lower(email) = lower(coalesce(auth.jwt() ->> 'email', '')));

create or replace function request_organization_membership(
  p_org_code text,
...
begin
  ...
  if not exists (select 1 from organizations where org_code = p_org_code) then
  ...
  insert into user_organizations (
    user_id, org_code, status, requested_at, requested_role, request_message,
    approved_at, approved_by, decision_note, decided_at, revoked_at
  ) values (
    v_user_id, p_org_code, 'pending', now(), nullif(trim(p_requested_role), ''),
    nullif(trim(p_request_message), ''), null, null, null, null, null
  )
  on conflict (user_id, org_code) do update
    set status = 'pending', requested_at = now(), requested_role = excluded.requested_role,
        request_message = excluded.request_message, approved_at = null, approved_by = null,
        decision_note = null, decided_at = null, revoked_at = null
...
begin
  ...
  if v_invitation.expires_at <= now() then
    update organization_invitations set status = 'expired', responded_at = now()
  ...
  if exists (
    ...
    where user_id = v_user_id and status = 'approved' and org_code <> v_invitation.org_code
  ) then
  ...
  insert into user_organizations (
    ...
    v_user_id, v_invitation.org_code, 'approved', v_invitation.created_at,
    v_invitation.invited_role, v_invitation.invitation_note, now(),
    v_invitation.invited_by, 'Accesso accettato tramite invito', now(), null
  )
  on conflict (user_id, org_code) do update
    set status = 'approved', requested_role = excluded.requested_role,
        request_message = excluded.request_message, approved_at = now(),
        approved_by = v_invitation.invited_by,
        decision_note = 'Accesso accettato tramite invito', decided_at = now(), revoked_at = null;

  update organization_invitations
  set status = 'accepted', responded_at = now(), accepted_by = v_user_id
  where id = p_invitation_id;
...
begin
  ...
  update organization_invitations set status = 'declined', responded_at = now()
  where id = p_invitation_id and lower(email) = v_email and status = 'pending';
  if not found then raise exception 'Invitation not found'; end if;
end;
$$;

create or replace function admin_decide_organization_membership(
...
begin
  ...
  select * into v_membership from user_organizations
  where id = p_membership_id and status = 'pending' for update;
  ...
  set status = p_outcome,
      ...
      decided_at = now(), decision_note = nullif(trim(p_decision_note), ''), revoked_at = null
  ...
  if p_outcome = 'approved' then
    ...
    where user_id = v_membership.user_id and status = 'pending' and id <> p_membership_id;
  end if;
  return p_outcome;
end;
$$;

create or replace function admin_revoke_organization_membership(
  p_membership_id bigint,
  p_admin_id uuid,
  p_decision_note text default null
)
...
begin
  if auth.role() <> 'service_role' then raise exception 'Service role required'; end if;
  update user_organizations
  set status = 'revoked', approved_at = null, approved_by = p_admin_id,
      decided_at = now(), decision_note = coalesce(nullif(trim(p_decision_note), ''), 'Accesso revocato'),
      revoked_at = now()
...
revoke all on function request_organization_membership(text, text, text) from public;
...
revoke all on function admin_decide_organization_membership(bigint, text, uuid, text) from public;
revoke all on function admin_revoke_organization_membership(bigint, uuid, text) from public;
grant execute on function request_organization_membership(text, text, text) to authenticated;
...
-- Private workbook v2, 2026-09-23. Standalone migration must only run once.
