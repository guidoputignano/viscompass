...
test('missing intermediate year cannot create an annual change or bridge',()=>{
 const rows=privatePillarAnalysis(fixture().filter(r=>r.year!==2024));
 assert.equal(rows[1].costYoy,null);
 assert.equal(rows[1].dddYoy,null);
 assert.equal(rows[1].bridge,null);
});
