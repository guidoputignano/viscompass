...
def main():
    ...
    manifest = json.loads((ROOT/'data/raw/aifa/aifa_series_manifest.json').read_text())
    sums = defaultdict(lambda: [Decimal(0),Decimal(0),0,0,0])
    sources, regions = [], {'000':'Italia'}
    for p in manifest['profiles']:
        path = ROOT / p['file'].replace('\\','/')
        raw = path.read_bytes()
    ...
    for year in range(2019,2026):
        path=ROOT/f'data/raw/denominators/istat/POSAS_{year}_it_Tutti_i_file.zip'
        raw=path.read_bytes()
    ...
    for (year,month,region,g,channel),v in sorted(sums.items()):
        ...
        a[5].add(month)
        if region=='000': monthly.append(dict(year=year,month=month,group=g,channel=channel,spend=float(v[0]) if v[2] else None,packs=float(v[1]) if v[3] else None))
    ...
    activity=list(csv.DictReader((ROOT/'data/derived/pillar_a/sdo_acute_regional_2021_2024.csv').open(encoding='utf-8')))
    for r in activity:
        for k in ('year','ordinary_days','day_accesses','days_plus_accesses'): r[k]=int(r[k])
    public_candidates=[]
    for r in csv.DictReader((ROOT/'outputs/pillar-a-scope/candidate_and_coverage.csv').open(encoding='utf-8-sig')):
        public_candidates.append({k:r[k] for k in ('atc5','group','ingredient_or_reference_names','aware','aware_status','public_parent_years')})
    data=dict(version='2026-09-20.1',scope=policy,regions=regions,annual=records,monthly=monthly,activity=activity,candidates=public_candidates,sources=sources,
              ...
              notes=['Spending and package sums use reported numeric cells; missing cells are counted, not filled.',
                     ...
                     'Separate provincial population is unavailable in staged regional tables; Bolzano/Trento rates remain null.',
