...
export function PrivateProductCharts({abc,series,names}:{abc:ProductAbcRow[];series:Atc5Series[];names:Record<string,string>}){
 ...
 return <section className="space-y-5 rounded-xl border bg-card p-5">
  ...
  <section className="rounded-lg border bg-secondary/30 p-4 space-y-3">
   <h3 className="font-semibold">Profilo regolatorio italiano · AIFA/Ministero</h3>
   <p className="text-sm text-muted-foreground">Questa è una dimensione distinta dalla VEN clinica: fascia A/H/C descrive rimborsabilità e accesso al SSN, mentre il regime di fornitura descrive chi può prescrivere o dispensare. Non viene interpretata come Vital, Essential o Non-essential.</p>
   <div className="grid gap-3 sm:grid-cols-3 text-sm"><div><strong>Fascia A</strong><p className="text-muted-foreground">Rimborsabilità SSN per medicinali essenziali o cronici.</p></div><div><strong>Fascia H</strong><p className="text-muted-foreground">Uso e distribuzione in ambito ospedaliero secondo le regole AIFA.</p></div><div><strong>Fascia C/C-bis</strong><p className="text-muted-foreground">Non rimborsata dal SSN; non equivale a non essenziale sul piano clinico.</p></div></div>
   <p className="text-xs text-muted-foreground">La classificazione prodotto-per-prodotto sarà caricata dall’istantanea ufficiale AIFA e riconciliata per AIC. Fonti: <a className="text-primary underline" href="https://www.aifa.gov.it/en-US/web/guest/liste-dei-farmaci" target="_blank" rel="noreferrer">liste AIFA dei farmaci</a> · <a className="text-primary underline" href="https://www.salute.gov.it/new/it/tema/farmacie/omeopatici/" target="_blank" rel="noreferrer">Ministero della Salute</a>.</p>
  </section>
  <p className="text-sm text-muted-foreground">VEN clinica: nessuna classificazione approvata è attualmente disponibile per questa versione. Nessun prodotto è classificato automaticamente come non essenziale. ABC può quindi essere letto subito con il profilo regolatorio AIFA, mentre la matrice ABC–VEN resta sospesa fino all’approvazione di un panel clinico.</p>
 </section>;
}
