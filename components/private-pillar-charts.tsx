...
export function PrivatePillarCharts({facts,regional}:{facts:PrivateFact[];regional:boolean}){
 ...
 let used=0;
 const waterfall=chosen?[
  {name:'Riferimento',base:0,value:chosen.baseline,signed:chosen.baseline},
  {name:'Costo medio',base:Math.min(chosen.baseline,chosen.baseline+chosen.price),value:Math.abs(chosen.price),signed:chosen.price},
  {name:'Mix',base:Math.min(chosen.baseline+chosen.price,chosen.baseline+chosen.price+chosen.mix),value:Math.abs(chosen.mix),signed:chosen.mix},
  {name:'Interazione',base:Math.min(chosen.actual-chosen.interaction,chosen.actual),value:Math.abs(chosen.interaction),signed:chosen.interaction},
  {name:'Osservato',base:0,value:chosen.actual,signed:chosen.actual}]:[];
 ...
 return <div className="space-y-6">
  ...
  {regional&&<section className={panel}><h2 className="text-xl font-semibold">Confronto interno a DDD totali costanti · {year}</h2>
   ...
   {!chosen?<p>Confronto non disponibile: servono almeno due Aziende e DDD positive in ogni categoria.</p>:<><p>Waterfall: {label(chosen.org)}. Seleziona un’Azienda sopra per cambiare dettaglio.</p><div className="grid gap-6 xl:grid-cols-2">
    <div className="h-80"><ResponsiveContainer><BarChart data={waterfall}><CartesianGrid strokeDasharray="3 5"/><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis width={85} tickFormatter={n}/><Tooltip formatter={(_v,_name,p)=>money(p.payload.signed)}/><Bar dataKey="base" stackId="a" fill="transparent" tooltipType="none"/><Bar dataKey="value" stackId="a" name="CF €">{waterfall.map((r,i)=><Cell key={r.name} fill={i===0||i===4?'#617b91':colors[i-1]}/>)}</Bar></BarChart></ResponsiveContainer></div>
