...
export function PillarAFlowVisuals({annual,region,regionName,group,channel}:{annual:PublicAnnual[];region:string;regionName:string;group:string;channel:string}){
 ...
 return <section className="space-y-6 rounded-2xl border bg-card p-5 sm:p-6">
  <div><h2 className="font-display text-xl font-semibold">Composizione e variazione della spesa</h2><p className="mt-2 text-sm text-muted-foreground">{regionName} · fonti pubbliche AIFA · importi riportati in euro</p></div>
  ...
  <div><h3 className="font-semibold">Sankey · flussi e famiglie</h3><p className="mt-2 text-sm text-muted-foreground">Acquisti diretti e convenzionata, antibiotici J01 e antifungini J02A. Questo grafico comprende entrambe le famiglie e i flussi, indipendentemente dai filtri sopra. Le bande rappresentano quote di spesa, non trasferimenti o percorsi di pazienti.</p>
   {/* The selector sits with the chart it drives, not in the section header:
       it was previously top-right, separated from the plot by two paragraphs,
       which reads as page furniture rather than a control for this figure. It
       governs the waterfall below as well, which is why it says so. */}
   <label className="mt-4 flex flex-wrap items-center gap-3 text-sm font-semibold">Anno analisi
    <select aria-label="Anno analisi dei flussi" value={year} onChange={e=>setYear(Number(e.target.value))} className="rounded-lg border bg-background p-2">{years.map(v=><option key={v}>{v}</option>)}</select>
    <span className="font-normal text-muted-foreground">Vale per il Sankey e per il waterfall qui sotto.</span></label>
